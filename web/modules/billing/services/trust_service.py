...
class TrustService:
    ...
    async def deposit(self, client_id: int, amount: Decimal, description: str, user_id: int,
        ...
        self.db.add(txn)
        write_audit(self.db, "trust.deposit", "trust_transaction", txn.id, new_values={"client_id": client_id, "amount": str(amount), "balance_after": str(new_balance)}, user_id=user_id)
        self.db.commit()
        return txn
    ...
    async def disburse(self, client_id: int, amount: Decimal, description: str, user_id: int,
        ...
        txn = TrustTransaction(tenant_id=self.db.tenant_id, trust_ledger_id=ledger.id, client_id=client_id,
            ...
            transaction_date=transaction_date or date.today(), created_by_id=user_id)
        self.db.add(txn)
        write_audit(self.db, "trust.disburse", "trust_transaction", txn.id, new_values={"client_id": client_id, "amount": str(amount), "balance_after": str(new_balance)}, user_id=user_id)
        if new_balance < 0:
            write_audit(self.db, "trust.negative_balance_alert", "trust_ledger", ledger.id, new_values={"client_id": client_id, "balance": str(new_balance)}, user_id=0)
        self.db.commit()
        return txn
