...
from core.models.billing import Invoice, InvoiceMatter
...
class InvoiceService:
    ...
    async def generate_invoice(self, client_id: int, matter_ids: List[int], period_start: date, period_end: date, user_id: int) -> Invoice:
        ...
        write_audit(self.db, "invoice.generate", "invoice", invoice.id, new_values={"client_id": client_id, "total": str(grand_total)}, user_id=user_id)
        self.db.commit()
        return invoice
    ...
    async def send_invoice(self, invoice_id: int, user_id: int) -> bool:
        ...
        write_audit(self.db, "invoice.send", "invoice", invoice.id, new_values={"sent_to": client.email}, user_id=user_id)
        self.db.commit()
        return True
    ...
    async def void_invoice(self, invoice_id: int, user_id: int, reason: str = "") -> bool:
        ...
        write_audit(self.db, "invoice.void", "invoice", invoice_id, new_values={"reason": reason}, user_id=user_id)
        self.db.commit()
        return True
