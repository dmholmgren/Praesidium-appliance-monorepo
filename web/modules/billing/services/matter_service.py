...
class MatterService:
    ...
    async def create_matter(self, data: dict, user_id: int) -> Matter:
        ...
        self.db.add(matter)
        self.db.flush()
        write_audit(self.db, "matter.create", "matter", matter.id, new_values={k: str(v) for k, v in data.items()}, user_id=user_id)
        self.db.commit()
        return matter
    ...
    async def assign_timekeeper(self, matter_id: int, user_id: int, role: str, assigned_by: int) -> MatterTimekeeper:
        ...
        self.db.add(tk)
        write_audit(self.db, "matter.assign_timekeeper", "matter_timekeeper", tk.id, new_values={"matter_id": matter_id, "user_id": user_id, "role": role}, user_id=assigned_by)
        self.db.commit()
        return tk
