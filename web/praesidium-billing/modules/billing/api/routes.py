...
@router.post("/clients")
async def create_client(data: ClientCreate, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
@router.post("/matters")
async def create_matter(data: MatterCreate, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
    return await MatterService(db).create_matter(data.model_dump(exclude_none=True), user.id)

@router.post("/matters/quick-create")
async def quick_create(client_id: int = Body(...), name: str = Body(...), db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
    return await MatterService(db).quick_create_matter(client_id, name, user.id)

# Rates
@router.post("/rates")
async def set_rate(data: RateSet, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
async def create_entry(data: TimeEntryCreate, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
async def bulk_approve(entry_ids: List[int] = Body(...), db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
    return {"approved_count": await TimeEntryService(db).bulk_approve(entry_ids, user.id)}

@router.post("/time-entries/{entry_id}/reject")
async def reject_entry(entry_id: int, reason: str = Body("", embed=True), db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
# QC
@router.post("/qc/run")
async def trigger_qc(entry_ids: Optional[List[int]] = Body(None), db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
async def trigger_recon(date_from: str = Body(...), date_to: str = Body(...), db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
    from rq import Queue; from redis import Redis
...
async def generate_invoice(data: InvoiceGenerate, db: TenantSession = Depends(get_db), branding: BrandingService = Depends(get_branding), user=Depends(get_current_user)):
...
async def send_invoice(invoice_id: int, db: TenantSession = Depends(get_db), branding: BrandingService = Depends(get_branding), user=Depends(get_current_user)):
...
async def record_payment(data: PaymentRecord, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
async def payment_link(invoice_id: int, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
async def trust_deposit(data: TrustDeposit, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
...
async def trust_disburse(data: TrustDeposit, db: TenantSession = Depends(get_db), user=Depends(get_current_user)):
