...
class PaymentService:
    ...
    async def record_payment(self, invoice_id: int, amount: Decimal, method: str, user_id: int,
        ...
        write_audit(self.db, "payment.record", "payment", payment.id, new_values={"invoice_id": invoice_id, "amount": str(amount), "method": method}, user_id=user_id)
        self.db.commit()
        return payment
    ...
    async def write_off(self, invoice_id: int, user_id: int, reason: str) -> bool:
        ...
        invoice.status = "void"
        write_audit(self.db, "invoice.write_off", "invoice", invoice_id, new_values={"amount": str(write_off_amount), "reason": reason}, user_id=user_id)
        self.db.commit()
        return True
