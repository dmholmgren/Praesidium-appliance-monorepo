"""
Brand context helper for DMS templates.
Converts Chat 0's BrandingConfig object into a dict usable by
both Jinja2 templates (brand.docs_url) and code (brand.get()).
"""
...
# Default brand values — used when BrandingConfig is not loaded
_DEFAULTS = {
...
def get_brand(request: Request) -> dict:
    """
    Extract branding from request.state.branding and return a
    template-friendly dict with all brand.* attributes needed by base.html.
    Falls back to Praesidium defaults if branding is not loaded.
    """
    ...
    # Extract CSS vars from the branding object
    ...
    return {
        ...
        "addin_display_name": getattr(branding, "addin_display_name", "") or getattr(branding, "platform_name", "Praesidium"),
        "addin_icon_url": getattr(branding, "addin_icon_url", "") or "",
        "suppress_attribution": getattr(branding, "suppress_attribution", False),
        "portal_url": f"https://portal.{base_domain}" if base_domain else "",
    }
