...
def seed_rules(tenant_id: str, db) -> int:
    ...
    for rule_data in rules:
        ...
        existing = db.query(CourtRule).filter_by(
...
def seed_holidays(tenant_id: str, db) -> int:
    ...
    for h_data in holidays:
        existing = db.query(CourtHoliday).filter_by(
...
def seed_ai_rules(tenant_id: str, db) -> int:
    ...
    for rule_data in rules:
        existing = db.query(CourtAIRule).filter_by(
        ...
        # TODO: fix write_audit to match Chat 0 positional signature
        # write_audit(db, "seed", "court_ai_rules", rule.id)
        count += 1
...
def main():
    ...
    with get_tenant_session(args.tenant_id) as db:
        try:
