...
async def get_firm_wip_chart(scope: dict) -> dict:
    """
    Top N matters by WIP — unified query across ts_slips (Timeslips import)
    and time_entries (native Praesidium entries).
    Works for both migrated tenants and new onboarding tenants.
    If attorney_id is in scope, filters to that attorney only.
    ts_slips path: resolves matter via ts_clients nickname2 join + ts_timekeepers for attorney filter.
    time_entries path: has direct matter_id + user_id FK.
    """
    tenant_id = _tid(scope)
    req = scope.get("request")
    limit = int((req.query_params.get("limit", "12")) if req else "12")

    # Resolve attorney filter
    attorney_id = None
    if req:
        atty_param = req.query_params.get("attorney_id")
        if atty_param:
            try:
                attorney_id = int(atty_param)
            except (ValueError, TypeError):
                pass
    ...
    if attorney_id:
        ts_atty_join = """
                    JOIN ts_timekeepers tk
                        ON tk.source_id = s.source_id
                        AND tk.ts_tk_id = s.source_tk_id
                        AND tk.praesidium_user_id = :atty_id"""
        ts_atty_clause = ""
        te_atty_clause = "AND te.user_id = :atty_id"
        params["atty_id"] = attorney_id
        ...
        async with AsyncSessionLocal() as session:
            rows = await session.execute(sa_text(f"""
                SELECT
                    ...
                    SUM(wip.wip_value) AS wip_value,
                    SUM(wip.hours)     AS wip_hours
                FROM (
                    -- Source 1: Timeslips import
                    SELECT
                    ...
                    JOIN ts_clients tc
                        ...
                        AND trim(tc.tenant_id) = trim(:tid){ts_atty_join}
                    ...
                    -- Source 2: Native time entries
                    SELECT
                        m2.matter_number,
                        COALESCE(te.amount, te.hours * COALESCE(te.rate, 0)) AS wip_value,
                        te.hours
                    FROM time_entries te
                    JOIN matters m2
                        ON te.matter_id = m2.id
                        AND trim(m2.tenant_id) = trim(:tid)
                    WHERE trim(te.tenant_id) = trim(:tid)
                      AND te.billable = true
                      AND te.status NOT IN ('billed', 'invoiced')
                      AND COALESCE(te.amount, te.hours * COALESCE(te.rate, 0)) > 0
                      {te_atty_clause}
                ) wip
                JOIN matters m
                    ON m.matter_number = wip.matter_number
                    AND trim(m.tenant_id) = trim(:tid)
                    AND m.status = 'active'
                GROUP BY m.id, m.matter_name, m.matter_number, m.client_id
                HAVING SUM(wip.wip_value) > 0
...
async def get_atty_deadlines(scope: dict) -> dict:
    ...
    atty_clause = ""
    params: dict = {"tid": tenant_id, "today": today,
                    "end": window_end}
...
async def get_firm_ar_pie(scope: dict) -> dict:
    """
    AR broken down by matter — top 8 matters by outstanding AR.
    Joins ts_invoices → ts_slips → ts_clients → matters.
    Returns matters list with ar_balance + client_id for navigation.
    """
    ...
    try:
        async with AsyncSessionLocal() as session:
            rows = await session.execute(sa_text("""
                SELECT
                    ...
                    m.matter_name,
                    m.matter_number,
                    SUM(ti.net_due)   AS ar_balance
                FROM ts_invoices ti
                JOIN ts_slips s
                    ON s.invoice_num = ti.invoice_num
                ...
                WHERE trim(ti.tenant_id) = trim(:tid)
                  ...
                  AND ti.net_due > 0
                  AND m.status = 'active'
                GROUP BY m.id, m.client_id, m.matter_name, m.matter_number
                HAVING SUM(ti.net_due) > 0
        ...
        total_ar = sum(m["ar_balance"] for m in matters)
        return {"matters": matters, "total_ar": total_ar, "error": None}

    except Exception as exc:
        import logging
        logging.getLogger(__name__).error("get_firm_ar_pie error: %s", exc)
        return {"matters": [], "total_ar": 0, "error": str(exc)}


async def get_firm_ar_pie_fixed(scope: dict) -> dict:
    """
    AR by matter — correct aggregation.
    Aggregates ts_invoices FIRST (DISTINCT invoice totals),
    then joins to matter via ts_clients nickname2.
    Avoids fan-out from multiple slips per invoice.
    """
    tenant_id = (scope.get("tenant_id") or "").strip()
        ...
        async with AsyncSessionLocal() as session:
            rows = await session.execute(sa_text("""
                WITH invoice_totals AS (
                    SELECT
                        s.source_client_id,
                        SUM(DISTINCT ti.net_due) AS ar_balance
                    FROM ts_invoices ti
                    JOIN ts_slips s
                        ON s.invoice_num = ti.invoice_num
                        AND trim(s.tenant_id) = trim(:tid)
                ...
                JOIN ts_clients tc
                    ON tc.ts_client_id = it.source_client_id
                    AND trim(tc.tenant_id) = trim(:tid)
                JOIN matters m
                    ON tc.ts_raw->>'nickname2' = m.matter_number
                    AND trim(m.tenant_id) = trim(:tid)
                    AND m.status = 'active'
                WHERE it.ar_balance > 0
                ORDER BY it.ar_balance DESC
                LIMIT 8
            """), {"tid": tenant_id})
