...
templates = Jinja2Templates(directory=["core/templates", "templates", "templates/ediscovery"])
...
async def _matter_access(matter_id: str, tenant_id: str) -> dict:
...
async def issue_map_view(request: Request, matter_id: str):
    ...
    matter = await _matter_access(matter_id, tenant_id)
...
async def create_issue_map_version(request: Request, matter_id: str):
    ...
    await _matter_access(matter_id, tenant_id)
...
@router.get("/matters/{matter_id}/issue-map/versions", response_class=HTMLResponse)
async def issue_map_version_history(request: Request, matter_id: str):
    ...
    nav = await get_nav_context(request, page='ediscovery')
    return templates.TemplateResponse(request, "ediscovery/issue_map_history.html", {
        "matter": matter,
        "matter_id": matter_id,
        "versions": versions,
        "nav": nav,
...
@router.post("/matters/{matter_id}/issue-map/trigger")
async def issue_map_trigger(request: Request, matter_id: str):
    """Manual refresh trigger — enqueues job and returns 202."""
    tenant_id = _get_tenant_id(request)
    await _matter_access(matter_id, tenant_id)

    try:
        from modules.ediscovery.issue_map import _enqueue_refresh
        ...
        log.error("issue_map trigger: enqueue failed: %s", exc)
...
@router.get("/matters/{matter_id}/drift-events", response_class=HTMLResponse)
async def drift_events_partial(request: Request, matter_id: str):
        ...
        rows = await session.execute(
            text("""
                SELECT id, dimension, drift_type, description,
                       magnitude, version_before, version_after,
    ...
    nav = await get_nav_context(request, page='ediscovery')
    return templates.TemplateResponse(request, "ediscovery/drift_events_partial.html", {
        "matter": matter,
        "matter_id": matter_id,
        "events": events,
        "nav": nav,
