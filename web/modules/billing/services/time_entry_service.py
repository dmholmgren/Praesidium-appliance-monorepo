...
class TimeEntryService:
    ...
    async def create_entry(self, data: dict, user_id: int) -> TimeEntry:
        ...
        self.db.add(entry)
        self.db.flush()
        write_audit(self.db, "time_entry.create", "time_entry", entry.id, new_values={"matter_id": str(data["matter_id"]), "hours": str(hours_rounded)}, user_id=user_id)
        self.db.commit()
        return entry
    ...
    async def update_entry(self, entry_id: int, data: dict, user_id: int) -> Optional[TimeEntry]:
        ...
        write_audit(self.db, "time_entry.update", "time_entry", entry.id,
            old_values={k: str(v) for k, v in old_values.items()}, new_values={k: str(v) for k, v in data.items()}, user_id=user_id)
        self.db.commit()
        return entry
    ...
    async def bulk_approve(self, entry_ids: List[int], user_id: int) -> int:
        ...
        for eid in entry_ids:
            ...
            if entry and entry.status in ("draft", "ai_suggested", "reviewed"):
                ...
                entry.updated_at = datetime.utcnow()
                write_audit(self.db, "time_entry.approve", "time_entry", eid, user_id=user_id)
                count += 1
        self.db.commit()
        return count
    ...
    async def reject_entry(self, entry_id: int, user_id: int, reason: str = "") -> Optional[TimeEntry]:
        ...
        entry.reviewed_at = datetime.utcnow()
        entry.updated_at = datetime.utcnow()
        write_audit(self.db, "time_entry.reject", "time_entry", entry_id, new_values={"reason": reason}, user_id=user_id)
        self.db.commit()
        return entry
