...
async def timesheet_review(
    ...
    async with AsyncSessionLocal() as db:
        ...
        pending_row = await db.execute(
            text("""
                SELECT COUNT(*) AS cnt FROM timesheet_drafts
...
@router.post("/{session_id}/ai-escalate")
async def ai_escalate(
    request: Request,
    session_id: str,
):
    """Opt-in AI matching for unmatched drafts.
    Stub — returns 0 matched until AI wiring is complete. No tokens burned."""
    sess = await _require_admin(request)
    try:
    ...
    async with AsyncSessionLocal() as db:
        row = await db.execute(
            text("""
                SELECT COUNT(*) AS cnt FROM timesheet_drafts
                WHERE session_id = :sid AND ai_confidence = 0
                  AND status = 'pending'
            """),
            {"sid": session_id},
        )
    ...
    # TODO: Wire AI escalation when ready — see §5.5 step 3 in ChatPrompts v7.5
    return JSONResponse({
        "status": "stub",
        "message": "AI escalation not yet wired. Use manual assignment for now.",
