...
async def ediscovery_chat(
    ...
    try:
        ...
        return JSONResponse(
            {"reply": f"I encountered an error connecting to the AI service. Please check your API key in Firm Settings. Error: {str(e)[:100]}"},
            status_code=200
        )
    ...
    import re
    display_reply = re.sub(r'\{[^{}]*"search_terms"[^{}]*\}', '', reply, flags=re.DOTALL).strip()
